<template>
  <el-container>
    ...
    <p>总GPA为：{{totalGPA}}{{semesterGPA !== '' ? '，本学期GPA为：'+semesterGPA: ''}}</p>
  ...
  methods: {
    ...
    async updateSemester() {
      ...
      let totalGPA = await this.$axios.get(
        `http://localhost:8000/api/queryTotalGPA?stu_id=${this.id}`
      );
      if (totalGPA.data.length > 0)
        this.totalGPA = totalGPA.data[0].averageGPA;
    },
    async selectSemester() {
      await this.updateGPA();
      let semesterGPA = await this.$axios.get(
        `http://localhost:8000/api/querySemesterGPA?stu_id=${this.id}&sem=${this.semester}`
      );
    ...
    async updateGPA() {
      let GPA = await this.$axios.get(
        `http://localhost:8000/api/queryCourseGPA?sid=${this.id}&sem=${this.semester}`
      );
